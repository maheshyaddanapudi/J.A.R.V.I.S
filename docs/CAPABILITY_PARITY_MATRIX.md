...
| G2 | Upgrades/extends its own capabilities (implied: evolving suit software IM1→AoU) | Two-stage self-extension: Stage A generate-without-activate → security check-in → Stage B controlled activation, versioned, rollbackable | **REAL** | 3 | HIGHEST-RISK subsystem; dedicated check-in mandatory (R-CAP-05). **Stage A built+verified 2026-07-17:** hard limit (R-CAP-08), guard, registry, generate-without-activate. **Stage B built+verified 2026-07-19 (D-0073 APPROVED):** activation as a `capability:<name>` gated tool that COMPOSES existing gated tools (never manifest code, never Z1); R-CAP-08 re-validated at activation; propose→approve→activate (J.A.R.V.I.S. proposes on its own initiative, user approves through any interface via the CONSEQUENTIAL activate gate — nothing self-activates); deactivation always available; durable across restart. Live: malicious capability REJECTED + never activatable; benign one activated → composition ran through the gated loop; heartbeat can propose but not activate. **Authoring closed 2026-07-19 (chat-parity audit):** gated `selfext.draft` lets J.A.R.V.I.S. AUTHOR composition-only capabilities itself + `selfext.recordGap` records gaps; hard limit now scans compositions terminally; CC `/selfext` gained the Stage-B approve+activate surface. Full loop live-verified 11/11 (gap→draft→scan→propose→deny-heartbeat→approve→activate→use, audit chain intact). **Real-brain day-in-the-life 2026-07-19:** J.A.R.V.I.S. drafted+activated+ran a capability from CHAT; a genuinely autonomous multi-beat heartbeat wrote its OWN agenda item and completed it on the next beat while deferring a non-urgent alert for quiet hours (all unstaged; `docs/verification/LIVING_WITH_JARVIS_2026-07-19.md`). **Novel CODE works in-container (D-0074, correction):** J.A.R.V.I.S. writes a new program via its gated file tools + runs it via the gated `bash -lc` terminal, and a self-written program becomes a reusable `capability:<name>` (composing `terminal.run` over it — safe subprocess, never in-process). Live: it authored a twin-prime sieve → activated → the capability computed the primes; a `rm -rf /` capability was refused by the terminal DENY policy. NOT Mac-dependent. Remaining (hardening, not core): heavier isolation sandbox + SBOM/license scans for fully-untrusted generation at scale; observe-first/rollback/versioning; never loading self-written code as a native in-process tool (deliberate Z1 boundary). |
