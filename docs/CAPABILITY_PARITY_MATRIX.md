...
| C2 | Operates the computer for Tony (implied constantly) | Full macOS control: Accessibility UI-tree, apps, windows, clipboard, keyboard/mouse, files, terminal, AppleScript/JXA, Shortcuts | **REAL** | 1 (one reversible action) / 2 (full) | Semantic-first; approval-gated; audited; reversible where possible. **Phase-2 foundation built 2026-07-17:** typed control HAL + SIMULATION adapter verified through the gated loop in-container (10 tests, live approve/deny/audit); real macOS adapter source (`MacDesktop.swift`, AXUIElement/CGEvent) written; real-adapter activation gated on the "enable computer control" check-in (D-0022). |
| C3 | Browses/researches across the public web (IM3 Mandarin investigation) | Browser automation + research pipeline with per-claim provenance | **REAL** | 2 | Playwright/CDP; domain allowlists; untrusted-content envelopes. **Built 2026-07-17 (D-0034):** REAL headless-browser capability (`kernel/src/web/`) — `web.open` (CONSEQUENTIAL, outward, per-navigation approval) + `web.readText`/`links`/`screenshot` (READ_ONLY, content → agent) + `web.fill`/`click`; network policy (http/https only, offline refuses external, optional allowlist); page content never audited. 8 tests + live + harness `P-WEB-01`. Remaining C3: per-claim provenance/citation pipeline, untrusted-content extraction hardening. |
...
| G2 | Upgrades/extends its own capabilities (implied: evolving suit software IM1→AoU) | Two-stage self-extension: Stage A generate-without-activate → security check-in → Stage B controlled activation, versioned, rollbackable | **REAL** | 3 | HIGHEST-RISK subsystem; dedicated check-in mandatory (R-CAP-05). **Foundation built+verified 2026-07-17:** hard limit (R-CAP-08), guard, registry, Stage A generate-without-activate — NO activation path (D-0023). Live: malicious capability touching the trust core rejected; benign one reaches awaiting_review without activating. |
