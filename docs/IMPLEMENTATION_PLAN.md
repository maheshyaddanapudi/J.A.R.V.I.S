...
| **Current slice** | **1.3 Voice pipeline — part 1 done in container (2026-07-17); Mac part next.** Done: jarvis-ears service with typed engine contracts; openWakeWord "hey jarvis" wake VERIFIED end-to-end (detects Kokoro-spoken wake phrase, ignores other speech); Silero VAD v6 (with required 64-sample context) verified; Kokoro TTS (4 en-GB voices) verified; optional key-gated OpenAI TTS adapter written (unverified — no key here); /health /tts /listen API live. Mac part remaining: streaming STT (Kyutai MLX + whisper.cpp), VPIO echo-cancel + barge-in via Swift bridge, latency metrics, expressive-TTS listening test → voice identity (D-0004a). Voice demo round 1 held: user finds Kokoro monotone; likes George/Daniel timbre. Earlier: 1.1 ✅, 1.2 ✅ (see git history; Mac acceptance of `make dev` + real-Ollama swap pending). |
