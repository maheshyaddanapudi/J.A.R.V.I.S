...
| **Current slice** | **1.6 Memory ✅ (container-verified 2026-07-17). Container-buildable Phase-1 slices are done (1.1, 1.2, 1.4, 1.5-in-1.4, 1.6); remaining work is Mac-gated.** 1.6: conversation + preference stores with epistemic-status/provenance/confidence/sensitivity; remember/view/correct/delete/pin/forget/export; secret refusal; memory.remember tool in the loop; **verified survives kernel restart (AT1.11)**, history preserved, audit intact (48 entries). CC shows live memory panel with forget. 35 kernel tests pass. Prior: 1.4 trust core (audit/policy/e-stop/approvals/loop), 1.2 gateway, 1.1 scaffold, 1.3 part-1 jarvis-ears. **Pending Mac work (needs the M3 Max):** 1.3 streaming STT (Kyutai MLX) + VPIO barge-in + latency metrics + expressive-voice test (D-0004a); 1.7 design-system check-in + CC hardening; 1.8 Tauri companion + Swift bridge + packaging; `make dev` Docker acceptance; real-Ollama provider swap; full offline AT1.12 with local voice. |
