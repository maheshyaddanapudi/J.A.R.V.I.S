...
| **Current slice** | **1.3 part 2 ✅ — full local voice round-trip works in-container (2026-07-17).** Real streaming STT (sherpa-onnx zipformer, CPU) added; `/voice-turn` runs audio→STT→kernel gated loop→TTS audio out, verified end-to-end (spoken command transcribed perfectly, reasoned, answered in real speech, turn persisted to memory); turn-taking/barge-in state machine built+tested. 9 ears tests + 35 kernel tests pass. **Container-buildable Phase-1 is essentially complete** (1.1, 1.2, 1.4, 1.5-in-1.4, 1.6, 1.3 wake/VAD/STT/TTS/turn-taking + AudioIO HAL). Added 2026-07-17: **AT1.12 offline path verified** (voice-turn with zero external egress); AudioIO HAL (typed contract + tested buffer adapter + macOS VPIO Swift bridge source in `apps/companion/swift/`). **Mac-gated remainder only:** live mic/speaker device *binding* + VPIO echo-cancel (`swift run JarvisAudio` — source written), Tauri app shell + packaged app (AT1.1), expressive-voice identity pick (D-0004a), real-audio latency metrics, `make dev` Docker acceptance. |
