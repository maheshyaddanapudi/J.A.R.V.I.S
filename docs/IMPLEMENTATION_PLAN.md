...
- **Slice 2.5 research-with-provenance ✅ (2026-07-17, container-verified, D-0036):** `kernel/src/research/` — `research.gather(query, urls[])` composes the gated web browser into ONE CONSEQUENTIAL sourced-evidence action (all URLs disclosed upfront; out-of-policy URL → clean pre-approval denial; per-URL policy applies inside gather). Returns query-relevant passages each tagged `{url, title, line, snippet}` — per-claim provenance (parity C3, the IM3-style investigation) — fed to the agent to cite; a refused source is recorded, never fabricated. 6 tests (real Chromium over a local server) + live + harness `P-RESEARCH-01`. Remaining Phase-2: screen understanding (ScreenCaptureKit — Mac), a per-source untrusted-content envelope + citation-check pass, the full encrypted memory store set.
