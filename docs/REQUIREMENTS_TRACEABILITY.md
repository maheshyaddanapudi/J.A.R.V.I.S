...
| ID | Requirement | Spec § | Parity | Phase | Acceptance | Status | Decisions |
|---|---|---|---|---|---|---|---|
| R-CAP-01 | No fixed connector list; registries for 14 entity kinds | 5.1 | G1, G6 | 3 | AT3.* | **PARTIAL 2026-07-17** — 7 registries built: tools (`core/tools`), models (gateway config), MCP servers (`mcp/`), capabilities (`selfext/`), devices (`devices/`), **skills** (`skills/`, migration 0009, D-0031), **prompts** (`prompts/`, migration 0013, D-0043 — persona now user-editable data, read by the conversation loop with a fallback). Remaining kinds (rules/workflows/agents/plugins/integrations/simulators/displays/sensors) follow the same shape as needed per phase | D-0031, D-0043 |
