...
| ID | Requirement | Spec § | Parity | Phase | Acceptance | Status | Decisions |
|---|---|---|---|---|---|---|---|
| R-MODEL-01 | Separately routable roles (15 listed) | 3.1 | — | 1 (subset) → 2+ | slice 1.2 tests | **BUILT+VERIFIED 2026-07-18** — 16 roles route independently via config file, env pins (`JARVIS_ROLE_<ROLE>`, D-0049), and **runtime overrides** (UI/API/gated tools, ledgered + persisted, D-0054/55); inspectable at `/gateway/roles` + `/models`; harness `P-MODELS-01` + `P-CONFIG-01` | D-0008, D-0049, D-0054, D-0055 |
| R-MODEL-02 | Providers: Anthropic/OpenAI-compat/Gemini/Ollama/custom; core decoupled from provider formats | 3.2 | — | 1 | provider-swap demo (§3.5) | **BUILT+VERIFIED 2026-07-18** — adapters: ollama, anthropic (current API: adaptive thinking + `output_config.effort`, D-0046), openai_compat (OpenAI/OpenRouter/Grok/vLLM/llama.cpp via baseUrl); ONE neutral settings vocabulary (`effort`/`thinking`) translated per provider inside adapters (D-0049) — wire formats never leak into core or user config; provider swap = config/pin change only (live-verified repeatedly); Gemini = future openai_compat/adapters entry | D-0008, D-0046, D-0049 |
| R-MODEL-03 | Discovery, structured-output validation, cost/latency tracking, privacy classification, fallback, pinning, per-agent policies | 3.3 | — | 1–2 | slice 1.2 tests | **BUILT+VERIFIED 2026-07-18** — ajv structured-output validation; `model_calls` audit (tokens/latency/outcome, never content) surfaced at `/gateway/calls` + `/models` (D-0047); LOCAL_ONLY/STANDARD privacy gating; pre-stream-only fallback (no silent mid-answer model switch); pinning via env + runtime overrides; per-role targets = per-agent policy substrate | D-0008, D-0047, D-0054 |
| R-MODEL-04 | Local-capable-first routing; no large-local mandate; fully offline when configured | 3.4 | G7 | 1 | AT1.11–12 | **BUILT+VERIFIED 2026-07-17** — role targets ordered local-first; `JARVIS_OFFLINE=1` verified end-to-end (text loop): local role streams real tokens, remote-only role refused ("offline mode: no local provider"), remote providers report "disabled: offline mode", **zero external TCP egress** during a converse. In `scripts/acceptance_platform.py` (P-OFFLINE-01). Voice path offline = AT1.12. **REFINED 2026-09-12** (G-25, D-0084): a retryable pre-stream failure (429 / 5xx / unreachable) is retried on the SAME target with jittered exponential backoff before the fallback chain moves on, every attempt its own `model_calls` row; knobs `gateway.retry.maxRetries` (2) and `gateway.retry.baseDelayMs` (750) catalogued (D-0053); found by a 14-minute OpenRouter rate-limit window in act three (107 failed calls on a single-target role) | D-0012, D-0084 |
...
| ID | Requirement | Spec § | Parity | Phase | Acceptance | Status | Decisions |
|---|---|---|---|---|---|---|---|
| R-CAP-01 | No fixed connector list; registries for 14 entity kinds | 5.1 | G1, G6 | 3 | AT3.* | **PARTIAL 2026-07-17** — 8 registries built: tools (`core/tools`), models (gateway config), MCP servers (`mcp/`), capabilities (`selfext/`), devices (`devices/`), **skills** (`skills/`, migration 0009, D-0031), **prompts** (`prompts/`, migration 0013, D-0043 — persona now user-editable data), **rules** (`proactive/rules.ts`, migration 0014, D-0044 — user-configurable proactivity, closed-set safe conditions, gated + suggestion-only). Remaining kinds (workflows/agents/plugins/integrations/simulators/displays/sensors) follow the same shape as needed per phase | D-0031, D-0043, D-0044 |
...
| R-MEM-07 | Entity recall seeds by **identity before similarity**: an entity whose name is in the query ranks ahead of vector neighbours, most-specific name first; works with no embedder | RETRIEVAL_FIDELITY_SPEC §4 | H1 | 2 | §4.5 unit + replay | **BUILT+VERIFIED 2026-09-01** — `recallGraph` identity seeds (`identityMatch`, longest name first) before similarity, `mode`/`seeds[].via`, seed facts ranked by query terms; knob `memory.recall.identityFirst`; 7 unit tests; replay over the 55 preserved Longitude-XL misses: true graph misses 2→0, seed[0]==asked entity 13→20 (+54 %); manual Sonnet-5 agent answers correct. Record `docs/verification/RETRIEVAL_FIDELITY_2026-09-01.md`. **FIELD-VERIFIED 2026-09-11** (Longitude-XL second act, 500 more simulated days on this build): attribute recall 93.0 % vs 87.9 % (+5.1 pp); the first-act dominant defect (vector neighbours answering exact-entity questions) is absent from the miss list; remaining read-side misses are twin substitution inside traversal (G-02/G-04). Record `docs/verification/LONGITUDE_XL_ACT2_2026-09-11.md`. **REFINED 2026-09-11** (R3/R10): the twin substitution's root cause was the entity resolver merging qualifier twins as aliases (G-17) — now a rule (`qualifierTwin`), merges audited + announced, folded twins split from evidence; look-alikes tagged DIFFERENT in graph/lookup output, "no connections recorded for X" explicit, the agent opens with not-found; day-1000 replay of the second act's 35 unique misses: 0 true graph misses, exact-seed precision 16→18 after the world-side reconciliations | D-0080 |
| R-MEM-08 | A correction reaches the store where the fact **actually lives** (entity fact or preference), supersedes with history, never creates a second home | RETRIEVAL_FIDELITY_SPEC §5 | H1, H2 | 2 | §5.3 unit + mini-life | **BUILT+VERIFIED 2026-09-01** — `memory.correct` resolution factId → fact text → preference (`matchKeys`, ties refused) → new fact behind a read-only probe; `route` reported; mini-life on Sonnet 5 found and fixed two more ways a good fact was lost (wrong-factId guard, new-statement-driven target instead of "most recent"); 11 unit tests; mini-life record `docs/verification/retrieval_fidelity/S4_minilife_sonnet5.md`. **FIELD-VERIFIED 2026-09-11:** 133/133 `memory.correct` calls succeeded over days 501–1000 (0 ambiguous refusals after the `pickPreference` selector fix), every announced flip a supersession chain (170 history rows), chapter-two facts written by this build recall at 97.5 %; the write guard refused 6 update-in-disguise `rememberFact` calls, each followed by a correct `memory.correct`. **REFINED 2026-09-11** (R2/R4/R5): first-person statements refused as entity facts with the preference write handed back (G-05); `reconcileHomes` retires the older of two homes (fact/preference/attribute clause) with history in the sleep cycle (G-03, 12 conflicts on the day-1000 world); exclusive relations supersede with history, migration 0027 (G-07). Mini-life on Sonnet 5: correction → one active home-city fact, clean answer | D-0080 |
| R-MEM-09 | Multi-statement teaching is available as **one verified batch call** with per-item outcomes; partial failure reported, never masked | RETRIEVAL_FIDELITY_SPEC §5 | H1 | 2 | §5.3 batch test | **BUILT+VERIFIED 2026-09-01** — `memory.rememberFacts(entity, statements[])` LOW_REVERSIBLE, each item written + re-read (`factById`), per-item `{stored, factId|error}`, `ok:false` on any failure with the items named, exact rollback; 5 unit tests. **FIELD-VERIFIED 2026-09-11:** 89/89 chapter-two facts delivered and read back over days 540–1000 (`expansion_delivered` 89). **REFINED 2026-09-11** (R6): single fact and preference writes also read back and quote the stored text (`readBack`, `ok:false` if not intact); statements that do not name their entity are stored with it (`withSubject`, G-18) | D-0080 |
| R-MEM-10 | Learning-by-correction promotes a topic only on **judge-confirmed** extractions; without a judge the system accumulates but does not promote, and says so | RETRIEVAL_FIDELITY_SPEC §6 | B4 | 2 | §6.3 | **BUILT+VERIFIED 2026-09-01** — candidates `{count, judged}`, promotion `count≥2 ∧ judged≥1`, fallback accumulate-only with the deferral notice in the decision `why`, legacy maps as judged 0, ledger cap 200, template subject-domain clause, `tuning` removed from `jarvis_xl` (audit 14574); 13 unit tests; manual Sonnet-5 record `docs/verification/retrieval_fidelity/S3_manual_sonnet5.md`. **FIELD-VERIFIED 2026-09-11:** learned topics constant at 7 from day 68 to day 1000 (no junk promotion in 500 days of forced-deep controls), 9 auto-escalations in the second act all on learned topics; gap G-09 — chapter two introduced no new deep-topic corrections, so late-life learning is untested until the third act. **R10 mini-life (2026-09-11):** two corrections promoted `cryogenic valves` (auto probe escalated as a learned topic) while the activity word `scheduling`, corrected twice with the same phrasing, was never promoted and stayed fast; chapter three schedules four late topics + two junk controls (T7) **REFINED 2026-09-13** (G-20, D-0085): the judge's abort cap is the catalogued `memory.judge.timeoutMs` read live — a cap below the judge model's real latency silently kept `judged` at 0 and disabled promotion entirely, so the cap is now a config knob with a safe fallback, not a build constant | D-0080, D-0085 |
| R-MEM-11 | A name the user declares for something ("X usually goes by Y") is an **identity**, not a fact: stored as an alias, matched on every read path, refused when ambiguous or a qualifier twin | — (found in the field, act three) | H1, H2 | 2 | alias unit + tool round trip | **BUILT+VERIFIED 2026-09-12** — `EntityMemory.addAlias/removeAlias` (audited, read back; refused for a clash with another active entity or a qualifier twin), `memory.alias` tool + `aliases` on `memory.rememberEntity`, `aliasInDisguise` refuses a naming statement offered as a fact with the alias call handed back, `aliasMatch` seeds identity recall on a declared alias at any length; a 4-letter handle resolves through `recall`, `recallGraph` and `memory.lookup`. Attempt 3 of act three measured the old path at 1/3 on `ravi` (G-26); attempt 4 measures T5 on this build | D-0084 |
| R-MEM-12 | Quiet-hours consolidation may merge two facts **only when they are about the same attribute**; every merge is audited per fact and announced by name; declined merges are counted | — (found in the field, act three) | H1 | 2 | consolidation unit | **BUILT+VERIFIED 2026-09-12** — `judgeMergeAllowed` (same parsed slot, or a shared content word beyond the entity name), `refused` returned and named in the report + timeline, `memory_consolidated.mergesRefused`, `fact_merged_by_consolidation` audit rows, `onMemoryChange('fact-merge')` announcements. Attempt 3 of act three measured the unguarded path: the night of day 1027 folded `theo eriksen`'s meeting day and location into the nickname fact (G-28), 3 merges in 31 nights against 10 in the 1,076 nights before **EXTENDED 2026-09-13** (G-21, D-0085): `reconcileArticleVariants()` folds two ACTIVE rows that differ only by a leading article into the plain name before the one-home pass — facts and relations migrate, the folded spelling stays an alias, audited and announced; a qualifier twin can never be folded | D-0084, D-0085 |
