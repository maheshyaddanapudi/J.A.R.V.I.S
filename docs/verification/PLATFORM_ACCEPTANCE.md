...
| P-SLEEP-01 sleep-cycle: learns from own record, bounded adjust, user override wins | PASS | J.A.R.V.I.S. journals its own routing decisions (0015, categorical only — no content) and consolidates them + the model-calls audit on demand (D-0051): evidence-backed findings (e.g. "you forced deep 4× while I chose it 0×"), bounded auto-adjustment (escalation threshold 1↔2, announced), proposals for consequential changes (never silently applied), and a structurally-enforced user-override contract — a manual setting with its reason is quoted back and never overridden. Consolidations land on the episodic timeline. **Live:** full contract exercised incl. respected override + timeline entries |
