...
|---|---|---|
| P-CORE-01 kernel health + migrations | PASS | status=ok, 12 migrations applied |
...
| P-MODELS-01 gateway observability: measured provider status + role table + call audit | PASS | `/gateway/status` returns live-measured provider reachability, `/gateway/roles` the routing table (with `@effort+thinking` annotations from config, D-0046), `/gateway/calls` the R-MODEL-03 audit tail — routing outcomes only, never message content. **Live (12/12):** the `/models` Command Center panel rendered all three over real state incl. an honest no-key failure row (D-0047) |
| P-REASON-01 deep-reasoning escalation: auto role switch + why + override + **learns topics** | PASS | J.A.R.V.I.S. decides when a turn warrants `deep_reasoning` (D-0048): explicit asks always escalate, two independent signals auto-escalate, the decision streams FIRST with its "why", the user can force deep/fast, and an ineligible deep provider degrades honestly (fast + explanation, never an error). Provider-agnostic — the role is served by whatever gateway config routes it to. **Learning (D-0050):** taught topics (`/core/reasoning/topics`) and ≥2 explicit-deep corrections promote topics that then escalate alone ("you've taught me to think deeply about 'X'"); stored as visible/deletable preferences; restart-surviving. **Live:** role switch visible in `/gateway/calls` served by a LOCAL model; teach→auto-escalate→forget verified in-harness; chat badge UI 6/6 |
| P-SLEEP-01 sleep-cycle: learns from own record, bounded adjust, override ledger | PASS | J.A.R.V.I.S. journals its own routing decisions (0015, categorical only — no content) and consolidates them + the model-calls audit on demand (D-0051): evidence-backed findings (e.g. "you forced deep 4× while I chose it 0×"), bounded auto-adjustment (escalation threshold 1↔2, announced), proposals for consequential changes (never silently applied), and the **D-0052 override contract** — a user pin (with its reason) is respected by default; contradicting evidence counted strictly since the pin can outweigh it against a higher, re-pin-scaled bar (6 → 12 → …), at which point J.A.R.V.I.S. may choose to change it, announcing the evidence. Consolidations land on the episodic timeline. **Live:** below-bar respect + above-bar change + re-pin bar-raise all exercised |
| P-CONFIG-01 runtime role overrides: live re-route + ledger + refuse unknown + restore | PASS | Role routing edits at runtime (D-0054, first D-0053 migration): `PUT /gateway/roles/:role` with canonical pins + a reason (ledgered: who/why/when), `/models` panel live editor, persistence across restarts (preference-backed, stale pins skipped honestly). Unknown providers refused — the egress surface cannot widen at runtime; privacy/offline gating applies downstream of overrides. **Live:** re-routed deep turn served by the overridden target; restart survival; UI 7/7 |
