...
| P-REASON-01 deep-reasoning escalation: auto role switch + why + user override | PASS | J.A.R.V.I.S. decides when a turn warrants `deep_reasoning` (D-0048): explicit asks always escalate, two independent signals auto-escalate, the decision streams FIRST with its "why", the user can force deep/fast, and an ineligible deep provider degrades honestly (fast + explanation, never an error). Provider-agnostic — the role is served by whatever gateway config routes it to. **Live:** role switch visible in `/gateway/calls` served by a LOCAL model; chat badge UI 6/6 |
