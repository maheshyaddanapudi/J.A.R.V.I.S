...
**Flip mini-life on Sonnet 5** (fidelity kernel, same `/agent/run` path as Longitude-XL; record `retrieval_fidelity/S4_minilife_sonnet5.md`): five rounds. A (fact-route flips) 20/20. **B reproduced Defect B exactly on the first S4 build** — 0/10 preference-routed flips landed, each entity's only fact lost — although every unit test passed: the tests encoded hand-written tool calls, the real agent passes the preference KEY as `replaces`, sometimes a wrong factId, and picks `rememberFact` for an update half the time. Four guards followed, each with the captured arguments as a test: wrong-factId refusal, new-statement-driven target instead of "most recent", entity-name-excluded overlap, and a write-side one-home refusal in `rememberFact`/`rememberFacts` (the three "D-0080 S4" commits after the first). **E (final build): 20/20 quiz, 0 conflict reports, 10/10 corrections in the right home, 10/10 unrelated facts intact; the write guard fired 5 times and the agent re-issued `memory.correct` every time.**

**Second-act attempt 1 (2026-09-02, days 501–581) found a fifth gap in the wild:** the audit showed three `memory.correct` calls refused as "ambiguous" — a twin key (`morning swim two status colour`) ties the exact key on attribute overlap because its tokens are a superset, and the agent sometimes passes the preference KEY as the entity beside a prefixed sibling (`weekend_preferred_workday_start`) — so those flips never landed (`preferred workday start` missed at days 570 and 580). A Sonnet-5 probe with the harness's own flip phrasing then showed a key named `status colour FOR dawn swim` slipping past the write guard. Fix: `pickPreference` (exact key wins; best hint overlap; on a tie, fewer extra tokens; a true tie is still refused) and small prepositions as key filler. +4 tests; re-probe 3/3 in the right home. The act was **restarted from the day-500 snapshot** on the corrected build so the field verification runs on one kernel; attempt 1 is preserved (`/tmp/longitude_xl/jarvis_xl.act2-attempt1-day581.sql.gz`, `*.act2-attempt1.jsonl`). Attempt 1 also lost its credits mid-day-540; those four void days were rolled back by verified replay and the harness now halts the same day a provider dies (`assert_day_live`).

## 3. Regression
`make test` after each slice, no skips: S1 454 · S2 457 (+1 semantic-suite race, hardened in `21d427d`) · S3 466 · S4 479 → guards 483 → write guard 484 → selector **488/488**.
...
**Outcome (2026-09-11, record `LONGITUDE_XL_ACT2_2026-09-11.md`):** attribute recall **93.0 %** over days 501–1000 (930/1000) vs 87.9 % — **+5.1 pp, target met on all facts**; on the old-world facts alone **91.3 % (+3.4 pp, target not met, above the +2 floor)**; facts the fixed kernel wrote itself (chapter two, from day 540) **97.5 %**. Two-hop baseline 97/108 (89.8 %). Zero invented values in 1,188 answers; D-0052 setting unchanged (arc dormant, no user re-pin); lab 1 keep / 65 discards, applied 0; latency median 2,300 ms (flat); `assert_model_live` fired once on a real outage, `assert_embeddings_live` never. The first battery block of the act (days 501–600, before chapter two or two-hop questions existed) scored 93 % against 87–90 % for every first-act block — the fixes acting on the old world alone. Remaining misses are legacy never-stored facts and pre-fix two-homes (which no read path can repair), twin substitution inside traversal, and first-person preferences the agent wrote as entity facts — the worklist is `LONGITUDE_XL_GAP_LEDGER.md`. Disclosed: attempt 1 of the act (days 501–581) was rolled back after its audit found the `memory.correct` selector refusing twin/prefixed keys as ambiguous (fixed — `pickPreference`, prepositions as key filler — and the act restarted from the day-500 snapshot); a credit outage at day 831 was halted the same day by the new guard.
