...
- Setup/operations (this repo): `./scripts/graphify-setup.sh` restores the CLI + key after a container restart (`--check` reports what is missing); `./scripts/graphify-refresh.sh` forces a full Sonnet-5 deep-mode rebuild. A PostToolUse hook refreshes the graph automatically after each edit. Details + the key-on-disk tradeoff: `docs/GRAPHIFY.md` (D-0078).
