...
`scripts/acceptance_platform.py` → **32 PASS · 3 verified-elsewhere · 4 NEEDS-MAC · 0 FAIL** (record: `docs/verification/PLATFORM_ACCEPTANCE.md`). All eight goal pillars have a real, verified surface: voice pipeline (offline round-trip), contextual awareness (`context/`), proactive behavior (`proactive/`, gated), cinematic UI (full Command Center + Voice Orb), macOS computer control (SIMULATION + real adapter, gated D-0022), device control (SIMULATION + HA adapter, gated D-0025), local encrypted memory + secrets vault, controlled self-extension (Stage-A hard limit). Plus the model gateway (pinned to the current Anthropic API — per-target `effort`/adaptive `thinking` as config, D-0046; observable via `/gateway/calls` + the `/models` panel, D-0047), **deep-reasoning escalation** (J.A.R.V.I.S. routes a hard turn to the `deep_reasoning` role, explained + overridable, provider-agnostic, D-0048; **learns** the user's deep topics from instruction + repeated correction, stored as visible/deletable preferences, D-0050), **provider-agnostic generation settings** (neutral `effort`/`thinking` translated per provider in the adapters; `JARVIS_EFFORT`/`JARVIS_THINKING`/`JARVIS_ROLE_<ROLE>` env controls, D-0049), **sleep-cycle consolidation** (J.A.R.V.I.S. journals its own routing decisions and consolidates them like a nightly reflection — bounded self-adjustment, proposals for the rest, user pin respected until the trail outweighs it, D-0051/D-0052; the same cycle now also tidies MEMORIES during quiet hours — duplicate facts merged with history, stale entities proposed-never-forgotten, live writes unaffected, D-0063), **runtime gateway role editor** (live re-route with ledger, `/models` panel + `PUT /gateway/roles/:role`, D-0054), **general runtime settings** (edit any catalogued knob live — effective = override-else-default, ledgered, Z1-excluded; `/settings` panel + gated `settings.set`/`reset`, D-0058), **background autonomy → living heartbeat** (bounded scheduler runs proactivity + sleep-cycle on a persisted, runtime-editable schedule — and now J.A.R.V.I.S. WRITES ITS OWN AGENDA and thinks on each beat within a hard risk ceiling: ≤LOW_REVERSIBLE auto, consequential auto-denied + queued for approval, every beat journaled + shown in /pulse; default-off, e-stop-halts; D-0024 + D-0064), **durable consent** (`always-allow-in-scope` grants persist across restart, listable/revocable, D-0059), MCP host, the **agent runtime** (multi-step plan-and-act through the gated loop, `agent/`), and **memory-evolution completeness** (agent-owned `memory.correct`/`forget` with read-then-write factId precision, near-duplicate settings guard, `settings.list`, A2UI cascade-prune — D-0062). **Night Lab (D-0079, 2026-08-28)** — evidence-gated self-experimentation, IMPLEMENTED + live-verified: during quiet hours the scheduler runs bounded experiments on a Z1-held allowlisted surface (`lab/surface.ts`, protected path) against an ISOLATED lab kernel + scratch DB, measured by the untouchable hash-stamped `bench/` (10 rubric conversations, 4 dims, 8 hard gates), keep protocol N=3/δ4/ε3-guard-bands, every attempt in the `lab_experiments` ledger, winners reach live only via the gated registries under three envelopes (auto / D-0052-pinned→ask / persona→always-proposal), ledger-derived morning report relayed in-chat (D-0077), `/lab` panel + `/lab/*` routes; first real scheduler-fired night: baseline 93.2, 4 honest discards, cap-halt; night 2 (live-surface null hypothesis fixed): a REAL KEEP earned against a user-set generic persona + the full proposal-envelope apply/revert cycle live through the panel — `docs/verification/NIGHT_LAB_2026-08-28.md`. 433 kernel + 9 ears tests pass (three-rhythm separation D-0065: live/heartbeat/quiet-hours never collide with each other or a live session). **The 4 NEEDS-MAC rows** (live voice I/O, packaged Tauri app, real macOS control, real Home Assistant) require the physical M3 Max + their activation gates — run `docs/MAC_BRINGUP.md` on the Mac. Pending check-ins: D-0004a, D-0022, D-0024, D-0025, D-0026, D-0027, D-0023.
...
- Setup/operations (this repo): `./scripts/graphify-setup.sh` restores the CLI + key after a container restart (`--check` reports what is missing); `./scripts/graphify-refresh.sh` runs the FULL pipeline (deep extraction + cluster + LLM community labels + graph.html). A PostToolUse hook refreshes the graph automatically after each edit — TRIMMED since 2026-08-28 (`--no-label --no-viz`: nodes/edges/communities stay current; run the refresh script for fresh community names/visuals). `.graphifyignore` excludes `docs/screenshots/` (vision calls on UI screenshots produced zero usable nodes). Details + the key-on-disk tradeoff: `docs/GRAPHIFY.md` (D-0078).
