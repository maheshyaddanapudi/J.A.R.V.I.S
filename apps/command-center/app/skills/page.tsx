...
interface Skill { id: string; name: string; description: string; objective: string; maxSteps: number; lastRunAt: string | null }
...
/**
 * Skills panel (R-CAP-01) — save named objectives and re-run them. Running a
 * skill executes its objective through the agent, so every step goes through the
 * gated loop; a consequential step surfaces an inline approval you resolve here.
 * A skill grants no new capability — it only names and reuses what the agent can
 * already do. Persistent e-stop halts a running skill.
 */
export default function SkillsPage() {
  ...
  const [result, setResult] = useState<{ skill: string; result: AgentResult } | null>(null);
  ...
  async function refresh() {
    try { setSkills((await fetch(`${KERNEL_URL}/skills`, { cache: "no-store" }).then((r) => r.json())).skills ?? []); } catch { /* */ }
  }
  ...
  async function create() {
    ...
    await fetch(`${KERNEL_URL}/skills`, {
      method: "POST", headers: { "content-type": "application/json" },
      body: JSON.stringify({ name: name.trim(), objective: objective.trim(), maxSteps }),
    });
    setName(""); setObjective("");
    void refresh();
  }
  async function del(id: string) {
    await fetch(`${KERNEL_URL}/skills/${id}`, { method: "DELETE" });
    void refresh();
  }
  async function run(s: Skill) {
    if (running) return;
    setRunning(s.id); setResult(null); setPipeline([]);
    try {
      const r = await fetch(`${KERNEL_URL}/skills/${s.id}/run`, {
        method: "POST", headers: { "content-type": "application/json" }, body: "{}",
      }).then((x) => x.json());
      setResult({ skill: s.name, result: r as AgentResult });
    } catch (err) {
      setResult({ skill: s.name, result: { objective: s.objective, answer: `[error: ${err instanceof Error ? err.message : String(err)}]`, steps: [], stepsUsed: 0, budgetExhausted: false, halted: false } });
    } finally {
      setRunning(null);
      void refresh();
    }
  ...
  return (
    <main style={{ padding: "1.5rem", maxWidth: 920, margin: "0 auto" }}>
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.6rem" }}>
        <div>
          <h1 style={{ fontSize: "1.05rem", letterSpacing: "0.2em", color: "var(--operational)", margin: 0 }}>J.A.R.V.I.S. — SKILLS</h1>
          <p style={{ color: "var(--dim)", margin: "0.2rem 0 0", fontSize: "0.8rem" }}>
            saved objectives, run via the gated agent · <a href="/" style={{ color: "var(--operational)" }}>dashboard</a> · <a href="/agent" style={{ color: "var(--operational)" }}>agent</a>
          </p>
        </div>
      ...
      {pending.length > 0 && (
        <div style={{ background: "var(--surface)", border: "1px solid var(--advisory)", padding: "0.6rem 1rem", marginBottom: "0.6rem" }}>
          <div style={{ color: "var(--advisory)", fontSize: "0.78rem", marginBottom: "0.3rem" }}>a step needs your approval:</div>
          {pending.map((p) => (
      ...
      <Panel tone="operational" title={`SKILLS (${skills.length})`}>
        {skills.length === 0 && <span style={{ color: "var(--dim)" }}>no skills yet — save one below</span>}
        {skills.map((s) => (
          <div key={s.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "0.5rem", marginBottom: "0.4rem" }}>
            <span>
              <span style={{ color: "var(--focal)" }}>{s.name}</span>{" "}
              <span style={{ color: "var(--dim)", fontSize: "0.72rem" }}>— {s.objective}</span>
            </span>
            <span style={{ display: "flex", gap: "0.3rem" }}>
              <button onClick={() => run(s)} disabled={running !== null || estop} style={{ ...btn("var(--operational)"), opacity: running || estop ? 0.5 : 1 }}>
                {running === s.id ? "running…" : "run"}
              </button>
              <button onClick={() => del(s.id)} style={btn("var(--critical)")}>delete</button>
            </span>
          </div>
        ))}
        <div style={{ display: "flex", gap: "0.3rem", flexWrap: "wrap", marginTop: "0.6rem" }}>
          ...
          <input value={objective} onChange={(e) => setObjective(e.target.value)} placeholder="objective (what should J.A.R.V.I.S. do)"
            style={{ flex: 1, minWidth: 220, ...inputStyle }} />
          <label style={{ color: "var(--dim)", fontSize: "0.7rem", alignSelf: "center" }}>
            steps <input type="number" min={1} max={20} value={maxSteps} onChange={(e) => setMaxSteps(Number(e.target.value))} style={{ width: 44, ...inputStyle }} />
          </label>
          <button onClick={create} style={btn("var(--operational)")}>save skill</button>
        </div>
      </Panel>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.8rem", marginTop: "0.8rem" }}>
        <Panel tone="advisory" title="GATED PIPELINE (live)">
          <div ref={feedRef} style={{ maxHeight: 220, overflowY: "auto", fontSize: "0.73rem" }}>
            {pipeline.length === 0 && <span style={{ color: "var(--dim)" }}>run a skill to watch it execute</span>}
            {pipeline.map((e, i) => (<div key={i} style={{ marginBottom: "0.25rem", color: pipeColor(e) }}>{formatEv(e)}</div>))}
          </div>
        </Panel>
        <Panel tone={result?.result.halted ? "critical" : "operational"} title="RESULT">
          {!result && <span style={{ color: "var(--dim)" }}>{running ? "…" : "no run yet"}</span>}
          {result && (
            <>
              <div style={{ color: "var(--dim)", fontSize: "0.72rem", marginBottom: "0.3rem" }}>
                {result.skill} · {result.result.stepsUsed} step(s){result.result.halted ? " · HALTED" : ""}
              </div>
              {result.result.steps.map((s) => (
                <div key={s.index} style={{ marginBottom: "0.2rem", fontSize: "0.76rem" }}>
                  <span style={{ color: s.denied || !s.ok ? "var(--critical)" : "var(--operational)" }}>
                    {s.denied ? "⊘" : s.ok ? "✓" : "✗"} {s.tool}
                  </span>
                </div>
              ))}
              <div style={{ color: "var(--focal)", marginTop: "0.4rem", whiteSpace: "pre-wrap" }}>{result.result.answer}</div>
            </>
          )}
        </Panel>
      </div>
